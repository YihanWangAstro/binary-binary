...
void binary_binary_Adrain(size_t th_id, std::string const &dir, size_t sim_num, double a_s, double E, double Q) {
  ...
  for (size_t i = 0; i < sim_num; ++i) {
    ...
    auto const in_orbit =
        orbit::HyperOrbit(M_tot(sun, jupiter), M_tot(star1, star2), V, B, w, 0.0, 0.0, r_start, orbit::Hyper::in);
    ...
    args.add_post_step_operation([&](auto &ptc, auto dt) {
      auto I_tmp = impulse(ptc[1], ptc[2], dt) + impulse(ptc[1], ptc[3], dt);
    ...
    args.add_stop_point_operation([&](auto &ptc, auto dt) {
      ...
      space::display(out_file, i, w, is_collided, a_j, e_j, a_s, e_s, I, It);
      out_file << std::endl;
...
void binary_binary_Adrain_v(size_t th_id, std::string const &dir, size_t sim_num, double a_s, double v_inf, double Q) {
  char name[100];
...
int main(int argc, char **argv) {
  ...
  for (double p = 0; p <= log10(max_Q); p += dp) {
    threads.emplace_back(std::thread{binary_binary_Adrain_v, th_id, output_dir, sim_num, a, v_inf, pow(10.0, p)});
    th_id++;
  }
